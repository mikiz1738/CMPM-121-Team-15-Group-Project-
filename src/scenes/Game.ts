...
export class Game extends Scene {
  ...
  serializeStateToByteArray() {
    const waterArray = new Float32Array(
      this.tileAttributes.flatMap((row) => row.map((tile) => tile.water)),
    );
    const sunlightArray = new Float32Array(
      this.tileAttributes.flatMap((row) => row.map((tile) => tile.sunEnergy)),
    );
    const plantArray = new Int32Array(
      this.tileAttributes.flatMap((row) =>
        row.map((tile) =>
          tile.plant
            ? this.plantTypes.findIndex((p) => p.name === tile.plant.name)
            : -1
        )
      ),
    );
    const growthStageArray = new Int32Array(
      this.tileAttributes.flatMap((row) =>
        row.map((tile) => tile.plant ? tile.plant.growthStage : 0)
      ),
    );
    const daysPlantedArray = new Int32Array(
      this.tileAttributes.flatMap((row) =>
        row.map((tile) => tile.plant ? tile.plant.daysPlanted : 0)
      ),
    );
    ...
    const totalSize = waterArray.byteLength + sunlightArray.byteLength +
      plantArray.byteLength +
      growthStageArray.byteLength + daysPlantedArray.byteLength +
      daysArray.byteLength + fullyGrownPlantsArray.byteLength;
  ...
  deserializeStateFromByteArray(byteArray: Uint8Array) {
    if (
      !this.tileAttributes || !Array.isArray(this.tileAttributes) ||
      !this.tileAttributes[0]
    ) {
      console.error(
        "tileAttributes is not properly initialized or has no rows",
      );
      return;
    }
    ...
    if (
      this.tileAttributes.length === 0 || this.tileAttributes[0].length === 0
    ) {
      console.error("tileAttributes grid is not initialized correctly");
      return;
    }

    const tileCount = this.tileAttributes.length *
      this.tileAttributes[0].length;

    const waterArray = new Float32Array(
      byteArray.buffer.slice(0, tileCount * 4),
    );
    const sunlightArray = new Float32Array(
      byteArray.buffer.slice(tileCount * 4, tileCount * 8),
    );
    const plantArray = new Int32Array(
      byteArray.buffer.slice(tileCount * 8, tileCount * 12),
    );
    const growthStageArray = new Int32Array(
      byteArray.buffer.slice(tileCount * 12, tileCount * 16),
    );
    const daysPlantedArray = new Int32Array(
      byteArray.buffer.slice(tileCount * 16, tileCount * 20),
    );

    // Extract days and fullyGrownPlants from the saved state
    const daysArray = new Int32Array(
      byteArray.buffer.slice(tileCount * 20, tileCount * 24),
    );
    this.days = daysArray[0];

    const fullyGrownPlantsArray = new Int32Array(
      byteArray.buffer.slice(tileCount * 24, tileCount * 28),
    );
  ...
  loadGameState(slot: number) {
    ...
    if (base64String) {
      ...
      console.log(
        `No saved game state found in slot ${slot}. Initializing empty state.`,
      );
  ...
  create() {
    ...
    this.background = this.add.tileSprite(0, 0, 1024, 768, "background")
      .setOrigin(0, 0);
    ...
    if (!tiles) {
      console.error(
        "Tileset 'dirt' not found. Check the preload key and file path.",
      );
      return;
    }
    ...
    if (
      ...
      this.tileAttributes = Array.from(
        { length: rows },
        () =>
  ...
  isNearTile(tileX: number, tileY: number): boolean {
    ...
    const tileCenterX = this.offsetX + tileX * this.tileWidth +
      this.tileWidth / 2;
    const tileCenterY = this.offsetY + tileY * this.tileHeight +
      this.tileHeight / 2;
  ...
  plant(tileX: number, tileY: number, plantType: number) {
    ...
    if (
      tile.water >= plant.requiredWater &&
      tile.sunEnergy >= plant.requiredSunEnergy
    ) {
    ...
    for (let y = 0; y < this.tileAttributes.length; y++) {
      for (let x = 0; x < this.tileAttributes[y].length; x++) {
        ...
        if (tile.plant) {
          ...
          const boostedGrowthTime = Math.max(
            plant.growthTime * growthMultiplier,
            1,
          );
  ...
  getNeighbors(tileX: number, tileY: number): { x: number; y: number }[] {
    ...
    for (const dir of directions) {
      ...
      if (
        nx >= 0 && ny >= 0 && ny < this.tileAttributes.length &&
        nx < this.tileAttributes[ny].length
      ) {
