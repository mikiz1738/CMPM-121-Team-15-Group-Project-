...
export class Game extends Scene {
    ...
    // Update serializeStateToByteArray to include daysPlanted per tile
    serializeStateToByteArray() {
        ...
        const totalSize = waterArray.byteLength + sunlightArray.byteLength + plantArray.byteLength +
            growthStageArray.byteLength + daysPlantedArray.byteLength + daysArray.byteLength;
    ...
    deserializeStateFromByteArray(byteArray: Uint8Array) {
        ...
        // Extract days from the saved state
        const daysArray = new Int32Array(byteArray.buffer.slice(tileCount * 20));
        this.days = daysArray[0]; // Restore the days value
